...
class Hand(object):
    def __init__(self, handSize, initialHandDict = None):
        ...
        if initialHandDict is None:
            ...
            for i in range(num_vowels, handSize):
                ...
                initialHandDict[x] = initialHandDict.get(x, 0) + 1
        #elif handSize != len(initialHandDict):
         #   raise ValueError('hand size is not equal with the length of initialHandDict')
    ...
    def update(self, word):
        ...
        for char in word:
            if self.handDict.get(char,0) == 0:
                return None
            elif self.handDict.get(char, 0) > 1:
                self.handDict[char] -= 1
            else:
                del self.handDict[char]
    ...
    def isEmpty(self):
        ...
        """
        isitempty = True
        for char in self.handDict.keys():
            if self.handDict[char] >= 1:
                isitempty = False
                break
            else:
                continue
    ...
    def __eq__(self, other):
        ...
        equV = True
        for char in self.handDict.keys():
            if char in other.handDict.keys() and self.handDict[char] == other.handDict[char]:
                continue
            else:
                equV = False
                break
